...
export class QwenAdapter implements AIEngineAdapter {
  ...
  constructor(private configService: ConfigService) {
    this.apiKey = this.configService.getQwenApiKey();
  }
  ...
  async diagnoseBrand(params: BrandDiagnosisParams): Promise<BrandDiagnosisResult> {
    ...
    try {
      ...
      console.error('Qwen诊断失败:', error.message);
  ...
  async generateContent(params: ContentGenerationParams): Promise<ContentGenerationResult> {
    ...
    try {
      ...
      console.error('Qwen内容生成失败:', error.message);
  ...
  async chat(params: ChatParams): Promise<ChatResult> {
    try {
      ...
      console.error('Qwen聊天失败:', error.message);
      return { message: { role: 'assistant', content: '抱歉，Qwen服务暂时不可用。' } };
    }
  }
