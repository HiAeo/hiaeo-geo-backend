...
let ApiKeyService = class ApiKeyService {
    ...
    async create(params) {
        ...
        const apiKey = this.apiKeyRepository.create({
            ...
            key,
            secret: hashedSecret,
            scopes: params.scopes || [api_key_entity_1.ApiKeyScope.ALL],
    ...
    async checkRateLimit(apiKey) {
        if (apiKey.usedCount >= apiKey.monthlyLimit && apiKey.monthlyLimit > 0) {
            throw new common_1.BadRequestException('月度请求限制已用完');
        }
        return true;
    }
    ...
    async findOne(id) {
        ...
        if (!apiKey) {
            throw new common_1.NotFoundException('API Key不存在');
        }
        return apiKey;
    }
