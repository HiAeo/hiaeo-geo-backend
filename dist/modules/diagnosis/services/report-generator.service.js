...
let ReportGeneratorService = class ReportGeneratorService {
    ...
    async generate(task, healthScore, competitorAnalysis, issueAnalysis) {
        const sections = [];
        const executiveSummary = await this.generateExecutiveSummary(task.brandName, healthScore, competitorAnalysis, issueAnalysis);
        sections.push({
            title: '执行摘要',
        ...
        sections.push({
            title: '优化建议',
            content: await this.generateSuggestionsSection(task.brandName, healthScore, issueAnalysis),
            type: 'action_plan',
        });
        const aiInsights = await this.generateAIInsights(task.brandName, healthScore, issueAnalysis);
        return {
    ...
    async generateExecutiveSummary(brandName, healthScore, competitorAnalysis, issueAnalysis) {
...
3. 制定持续优化计划`;
    ...
    generateDimensionAnalysisSection(healthScore) {
        let content = `# 各维度详细分析\n\n`;
        for (const dim of healthScore.dimensionScores) {
    ...
    async generateIssueSection(brandName, issueAnalysis) {
    ...
    generateCompetitorSection(analysis) {
        let content = `# 竞品分析\n\n`;
        content += `## 竞品对比\n\n`;
    ...
    async generateSuggestionsSection(brandName, healthScore, issueAnalysis) {
    ...
    async generateAIInsights(brandName, healthScore, issueAnalysis) {
        const prompt = `
